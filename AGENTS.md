...
This repo registers the `ctx-memory` MCP server for Claude Code (`.mcp.json`), Cursor
(`.cursor/mcp.json`), and Codex CLI (`.codex/config.toml` — run `codex trust` on this project
once for Codex to read it). Whichever agent you are, use its tools proactively:

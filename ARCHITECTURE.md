...
Registration (Claude Code, project-level `.mcp.json`):
...
Cursor, Codex, and Antigravity take the same `command`/`args` shape in their own MCP config —
the server itself doesn't know or care which client is on the other end of stdio.
